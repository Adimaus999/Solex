...
if __name__=="__main__":
    ...
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='range_estimates_no_ML'")
    table_exists = cursor.fetchone()
    
    # Create the table if it does not exist
    if not table_exists:
        create_table_query = '''
        CREATE TABLE IF NOT EXISTS range_estimates_no_ML (
            timestamp TEXT,
            ssRange REAL,
            csRange REAL,
            hsRange REAL,
    ...
    while True:
        ...
        insert_query = '''
        INSERT INTO range_estimates_no_ML (timestamp, ssRange, csRange, hsRange, optimalSpeed)
        VALUES (?, ?, ?, ?, ?)
        '''
