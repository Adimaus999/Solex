...
class MyApp(QMainWindow):
    def __init__(self):
        ...
        conn = sqlite3.connect('Minmal_Database.bd')
        cursor = conn.cursor()
        sqlquery = '''SELECT raceLength FROM battery_charge ORDER BY datetime(timestamp) DESC LIMIT 1'''
        cursor.execute(sqlquery)
        self.BatteryCharge = cursor.fetchone()[0]
        conn.close()
        logging.info(f"Initial Remaining: {self.BatteryCharge}")
    ...
    def SQLreadmessage(self, sensor_id, db_path="Minmal_Database.bd", table_name="sensor_logs"):
                """
    ...
    def SQLread(self, sensor_id, db_path="Minmal_Database.bd", table_name="sensor_data"):
        """
        Function to extract most recent sensor data from an SQL database according to the timestamp column. The function 
        returns the sensor data sepcified according to the input 'sensor_id'. If there is any error, the function returns
        a nan.
